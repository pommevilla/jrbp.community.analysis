...
```{r read_data}
taxa <- data.table(read.delim("./data/classified_otus"));colnames(taxa)<-c("OTU",colnames(taxa[,-1]))

head(taxa)

cooccurrence_data <- data.table(read.table("./data/jrbp_cooccurrence", 
...
```{r pda.2}
rough_tree <- read.tree("./data/jrbp_phylo_tree")
```
